...
class ThreadManualResetTester
{
    static void Main(string[] args)
    {
        ...
        ThreadPool.QueueUserWorkItem(printer1.Print!, random.Next(5001)); // 5001 milliseconds);

        ManualResetEvent e2 = new ManualResetEvent(false);
        MessagePrinter printer2 = new MessagePrinter(e2);
        ThreadPool.QueueUserWorkItem(printer2.Print!, random.Next(5001)); // 5001 milliseconds);

        ManualResetEvent e3 = new ManualResetEvent(false);
        MessagePrinter printer3 = new MessagePrinter(e3);
        ThreadPool.QueueUserWorkItem(printer3.Print!, random.Next(5001)); // 5001 milliseconds);
